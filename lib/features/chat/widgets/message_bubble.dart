                ...
                if (!isMe) ...[
                  if (isGroup)
                    if (isFirstSequence)
                      CircleAvatar(
                        radius: 14.r,
                        backgroundColor: AppColors.senderNameColor(message.senderId).withValues(alpha: 0.25),
                        backgroundImage: senderPhotoUrl != null && senderPhotoUrl.isNotEmpty
                            ? NetworkImage(senderPhotoUrl)
                            : null,
                        onBackgroundImageError: senderPhotoUrl != null && senderPhotoUrl.isNotEmpty ? (_, __) {} : null,
                        child: senderPhotoUrl == null || senderPhotoUrl.isEmpty
                            ? Text(
                                senderName.isNotEmpty ? senderName[0].toUpperCase() : '?',
                                style: textTheme.senderName.copyWith(
                                  color: AppColors.senderNameColor(message.senderId).withValues(alpha: 0.7),
                                ),
                              )
                            : null,
                      )
